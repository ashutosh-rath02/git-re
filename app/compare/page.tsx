...
const Compare = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        ...
        !suggestionRef1.current.contains(event.target as Node)
      ) {
        setShowSuggestions1(false);
      }
      if (
        suggestionRef2.current &&
        !suggestionRef2.current.contains(event.target as Node)
      ) {
  ...
  const fetchUserData = async (username: string): Promise<UserData> => {
    ...
    if (stats) {
      return {
        ...
        rating,
        rank: rank.user_rank,
        userJoinedDate: new Date(stats.userJoinedDate),
      };
    } else {
  ...
  const handleCompare = async () => {
    ...
    try {
      ...
      setError(
        "Either the username(s) does not exist in the leaderboard or is/are invalid."
      );
  ...
  const ComparisonTable = ({
    ...
    const compareStats = (
      stat1: number | string | Date,
      stat2: number | string | Date,
      key: keyof UserData
    ) => {
      if (
        key === "name" ||
          ...
          <tbody>
            {stats.map(({ label, key }) => (
              <tr key={key} className="border-b dark:border-gray-700">
                ...
                <td
                  className={`p-2 text-center ${compareStats(
                    userData1[key],
                    userData2[key],
                    key
                  )}`}
                >
                  {key === "name"
                    ? userData1[key] || userData1.login
                    : key === "rating"
                    ? (userData1[key] as number).toFixed(2)
                ...
                >
                  {key === "name"
                    ? userData2[key] || userData2.login
                    : key === "rating"
                    ? (userData2[key] as number).toFixed(2)
  ...
  const ComparisonBarChart = ({
    ...
    const maxValues = stats.reduce((acc, { key }) => {
      if (
        typeof userData1[key] === "number" &&
        typeof userData2[key] === "number"
      ) {
      ...
      <div className="flex items-center space-x-2 w-full">
        ...
        <div className="flex-grow h-6 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
          {maxValue > 0 && typeof userData[key] === "number" ? (
            <div
              className={`h-full ${
                userData === userData1 ? "bg-blue-500" : "bg-green-500"
              }`}
              style={{
                width: `${((userData[key] as number) / maxValue) * 100}%`,
              }}
            />
          ) : (
    ...
    return (
      <div className="w-full overflow-x-auto mt-10">
        <h2 className="text-xl font-bold mb-4">
          Comparison Bar Chart (Relative Comparison)
        </h2>
      ...
      <div className="flex flex-col space-y-4 sm:flex-row sm:space-y-0 sm:space-x-4 mb-6">
        <div className="flex-grow relative">
          ...
          {showSuggestions1 && (
            <div
              ref={suggestionRef1}
              className="absolute z-10 w-full bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-700 border mt-1 rounded-md shadow-lg"
            >
      ...
      {userData1 && userData2 && (
        <div className="mt-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <UserCard
              userData={userData1}
              isWinner={userData1.rank < userData2.rank}
            />
            <UserCard
              userData={userData2}
              isWinner={userData2.rank < userData1.rank}
            />
          </div>
